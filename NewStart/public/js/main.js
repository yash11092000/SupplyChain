...
var contract_address = "0xA198c4A76093E66Ef64c387E3835B1510e20D8eD";
var contract_abi = [
    {
        "inputs": [
            {
    ...
    CreBtn.addEventListener("click", function () {
        ...
        if (name.length != 0) {
            web3.eth.getAccounts().then(function (accounts) {
                account = accounts[0];
                console.log(account);
                contract.methods.CreatedProd(id, name, quantity, datetime).send({ from: account })
                    .then(function (result) {
                        console.log(result);
                        swal("Good job!", `You Product Is Created! and Your ID is ${id}`, "success");
            ...
            swal("OOPS!", "Try Entering Valid Id or Enter Some Text for Product Name", "danger");
        }
    });
}
...
if (fetchBtn) {
    ...
    fetchBtn.addEventListener("click", function () {
        ...
        web3.eth.getAccounts().then(function (accounts) {
            account = accounts[0];
            // console.log(account)
            contract.methods.GetProd(id).call({ from: account })
                .then(function (result) {
                    ...
                    ProdQuantity.innerText = "Your Product Quantity Is :" + result.ProdQty;
                    ProdCreatedDate.innerText = "Your Product Is Created at :" + result.CreatedDate;
                    ProdIsPicked.innerText = "IsProductPicked:" + result.IsPicked;
                    ProdPickedDate.innerText = "Your Product Picked at :" + result.PickedDate;
                    ProdDeliveredDate.innerText = "Your Product DeliveryDate :" + result.DeliveredDate;
    ...
    TriggerDel.addEventListener("click", function () {
        ...
        if (id > 0) {
            web3.eth.getAccounts().then(function (accounts) {
                account = accounts[0];
                console.log(account);
                contract.methods.TriggerDelivery(id, datetime).send({ from: account })
                    .then(function (result) {
                        console.log(result);
                        swal("Nice!", "Product JourNey Started!", "success");
                    }).catch(function (err) {
                        console.log("Sorry Some Error Occured OR Your Product ", id, "is Further in SupplyChain");
...
if (DeliveryStatus) {
    DeliveryStatus.addEventListener("click", function () {
        ...
        web3.eth.getAccounts().then(function (accounts) {
            account = accounts[0];
            console.log(account);
            contract.methods.DeliveredDate(id, datetime).send({ from: account })
                .then(function (result) {
                    console.log(result);
